...
class Classifier(nn.Sequential):
	def __init__(self, model_drug, model_protein, **config):
		...
		# print(self.input_dim_drug, self.input_dim_protein)
		# print(dims)
	...
	def forward(self, v_D, v_P):
		...
		# print(v_D.shape, v_P.shape, v_f.shape)
...
class DBTA:
	...
	def __init__(self, **config):
		...
		if target_encoding == 'AAC' or target_encoding == 'PseudoAAC' or  target_encoding == 'Conjoint_triad' or target_encoding == 'Quasi-seq' or target_encoding == 'ESPF':
			...
			self.model_protein = InspireEncoder(**config)
		else:
			raise AttributeError('Please use one of the available encoding method.')
	...
	def train(self, train, val, test = None, verbose = True, drop_last=False, save_path=None, ES_thresh=3):
